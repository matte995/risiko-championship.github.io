...
const tooltip = d3.select("#tooltip");
    ...
    d3.csv("../../../js/statistiche/chart3/total_points.csv").then(data => {
      // Parsing
      data.forEach(d => {
      ...
      function buildChart(selectedChampionship) {
        ...
        const minY = d3.min(filteredData, d => d.Punti_totali) - 10;
        const maxY = d3.max(filteredData, d => d.Punti_totali);
              ...
              .on("mouseover", (event, d) => {
                tooltip.style("opacity", 1)
                  .html(`
                    ...
                    ${d.Punti_totali} punti totali<br>
        ...
        const playerAverages = players.map(p => {
          ...
          const totalPoints = d3.mean(validGames, d => d.Punti_totali);
          return { Giocatore: p.Giocatore, Punti_totali: totalPoints ? totalPoints.toFixed(2) : "0.00" };
        });

        const playerAveragesAll = players.map(p => {
          // Calcola la media punti su tutte le partite giocate (incluse scartate)
          const avgPoints = d3.mean(p.values, d => d.Punti_totali);
          return { Giocatore: p.Giocatore, Punti_totali: avgPoints ? avgPoints.toFixed(2) : "0.00" };
        });
          ...
          .join("g")
            ...
            .on("mouseover", (event, d) => {
              ...
              .html(`
                  ...
                  Media punti : ${playerAverages.find(p => p.Giocatore === d.Giocatore).Punti_totali}<br>
                  Media punti globale: ${playerAveragesAll.find(p => p.Giocatore === d.Giocatore).Punti_totali}
                `)
                .style("left", (event.pageX + 10) + "px")
                .style("top", (event.pageY - 28) + "px");
              d3.selectAll(".line").style("opacity", 0.2);
